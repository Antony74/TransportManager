...
void draw()
{
  ...
  float nRadius = 0.15 * width;

  pushMatrix();
  PVector pvA = new PVector(0.3 * width, 0.2 * height);
  translate(pvA.x, pvA.y);
  locationMarker("A", nRadius);
  popMatrix();
  
  pushMatrix();
  PVector pvB = new PVector(0.7 * width, 0.5 * height);
  translate(pvB.x, pvB.y);
  ...
  PVector pvC1 = new PVector(pvB.x + (nRadius * 1.0), pvA.y);
  PVector pvC2 = new PVector(pvA.x - (nRadius * 3.0), pvB.y + (nRadius * 0.0));

  noFill();
  stroke(255,0,0);
  strokeWeight(0.05 * width);
  bezier(pvA.x, pvA.y, pvC1.x, pvC1.y, pvC2.x, pvC2.y, pvB.x, pvB.y);
