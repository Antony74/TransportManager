	...
	if (nSchemaLevel)
	{
		...
		for (UINT nField = 0; nField < vecFieldInfo.size(); ++nField)
		{
			...
			if (nSchemaLevel >= 2)
			{
				...
				oField->Set(String::New("ISAUTOINCREMENT"), Boolean::New(info.m_bIsAutoIncrement));
			}
				...
				for (UINT nOperation = 0; nOperation < arrOperations->Length(); ++nOperation)
				{
					...
					if (sOperationName == "add")
					{
						...
						for (UINT nFld = 0; nFld < arrNewRecord->Length(); ++nFld)
						{
							...
							_variant_t vtNewValue = ::AsVariant(oNewRecord->Get(vFieldname));

							try
							{
								rs->Fields->GetItem(vtFieldname)->PutValue(vtNewValue);
							}
							catch(_com_error &e)
							{
								bOK = false;
					...
					else if (sOperationName == "edit")
					{
						...
						if (bGotIndex == false)
						{
							...
							while (rs->adoEOF == false)
							{
								...
								{
									CString sError = _T("Can't perform update because this record could not be found.  (It may have already been changed or deleted)");
