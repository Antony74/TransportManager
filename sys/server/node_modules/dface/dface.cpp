...
_variant_t AsDateVariant(const Handle<Value> value)
{
	if (value->IsNumber())
	{
		double dDate = ::floor(value->NumberValue() + 0.5);
	...
	{
		_ASSERT(value->IsNull() || ::AsCString(value) == _T(""));
		return _variant_t();
	}
}
						...
						if (bGotIndex == false)
						{
							...
							while (rs->adoEOF == false)
							{
								...
								for (long nFld = 0; nFld < rs->Fields->GetCount(); ++nFld)
								{
									...
									if (vtOldValue.vt == VT_EMPTY)
									{
										vtOldValue.vt = VT_NULL;
									}
								...
								if (bThisRecordMatches)
								{
									for (long nFld = 0; nFld < rs->Fields->GetCount(); ++nFld)
									{
										...
										if (sFieldName != sAutoIncrementFieldName)
										{
											...
											case ADODB::adDBTime:
											case ADODB::adDBTimeStamp:
												vtNewValue = ::AsDateVariant(oNewRecord->Get(::AsV8String(sFieldName)));
												break;

											default:
												vtNewValue = ::AsVariant(oNewRecord->Get(::AsV8String(sFieldName)));
											}
