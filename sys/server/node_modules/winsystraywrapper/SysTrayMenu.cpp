...
CSysTrayMenu* CSysTrayMenu::Create(const CString& sIconPath, const CString& sTitle, HANDLE hWrappedProcess, CMenuObject& menuObject)
{
	...
	std::vector<CString>& vec = m_pSoleInstance->m_vecMenuItemCaptions;
	vec.clear();

	for (int n = 0; n < menuObject.GetLength(); ++n)
	{
		CString sCaption = menuObject.GetCaption(n);
		vec.push_back(sCaption);
	}
...
LRESULT CALLBACK CSysTrayMenu::WndProcImpl(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONUP:
		...
		if (m_hMenu == NULL)
		{
			...
			for (UINT nItem = 0; nItem < m_vecMenuItemCaptions.size(); ++nItem)
			{
				CString sCaption = m_vecMenuItemCaptions[nItem];
		...
		UINT nClickedItem = ::TrackPopupMenu(
								  m_hMenu,
		...
		if (nClickedItem >= 0 && nClickedItem < m_vecMenuItemCaptions.size())
		{
...
void CSysTrayMenu::Stop()
{
	...
	DWORD dw = ::WaitForSingleObject(m_hThread, 5000);
	::PostQuitMessage(0);
